...
template <typename S1, typename S2> using _merge_t = typename _merge<S1, S2>::integer_sequence;

template <typename T, size_t N> struct _generate;
template <typename T, size_t N> using _generate_t = typename _generate<T, N>::integer_sequence;
...
template <typename T> struct _generate<T, 0> : integer_sequence<T>
{
};
template <typename T> struct _generate<T, 1> : integer_sequence<T, 0>
{
};
}
template <typename T, size_t N> using make_integer_sequence = detail::_generate_t<T, N>;
