    ...
    def cli_h(self) -> str:
        ...
        def sequence_string_helper(seq: str) -> list:
            ...
            for i in range(len(seq_lens)):
                seq_lens_string = f"{seq_lens_string}{str(seq_lens[i])}"
                fs = str(repr(seqs[i])).strip("'").replace("\\\\", "\\")
                seqs_string = f'{seqs_string}"{fs}"'
                if i != len(seq_lens) - 1:
                    seq_lens_string = f"{seq_lens_string}, "
                    seqs_string = f"{seqs_string}, "
            seq_lens_string = f"{seq_lens_string}}}"
            seqs_string = f"{seqs_string}}}"
        ...
        # process parameters
        pprm = self.cli_options["process parameters"]["var"]
        ...
        for root_command_index in self.command_index:
            ...
            if int(self.command_index[root_command_index]["root index key"]) == int(
                ...
                fn = self.cli_options["commands"]["parameters"][
                    self.command_index[root_command_index]["parameters key"]
                ]["functionName"]
        ...
        loop_function = ""
        if (
            self.cli_options["builtin methods"]["var"]["outputToStream"] == True
            and stream_string != ""
        ...
        code_string = f"{code_string}{setup_h}"
        return code_string
