...
struct StartStopSequences
{
    ...
    ih_auto::max_num_start_stop_seq
        start_stop_sequence_lens[IH_MAX_NUM_START_STOP_SEQ]; ///< start stop sequence lens start_stop_sequence_lens
                                                             ///< [@link IH_MAX_NUM_START_STOP_SEQ @endlink]
    char start_stop_sequence_pairs[IH_MAX_NUM_START_STOP_SEQ]
...
struct CommandRuntimeCalc
{
    ...
    ih_auto::max_per_root_memcmp_ranges **memcmp_ranges_arr; ///< 2d array[row][col], each [row] is for one Parameters
                                                             ///< command string which contains wcc
};
// end group structs
///@}
}; // namespace ih
...
const char *const command_error_strings[] PROGMEM = {
    command_string,         root_pointer_warn,  wcc_warn,          cmd_len_warn,          cmd_len_dec_warn,
    cmd_len_inc_warn,       cmd_depth_inc_warn, cmd_subc_inc_warn, cmd_num_args_inc_warn, cmd_max_num_args_inc_warn,
    cmd_flip_num_args_warn, cmd_rejected_warn,
};
